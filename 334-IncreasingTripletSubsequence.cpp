...
//  main.cpp
//  leecode
/*  算法思路：要求出现3个数能递增，首先想到会出现两对分离的递增数据对即可，因为第三个数会比第二个数大，那么第三个数一定也在一个递增数据对里面，并且是这个数据对里的第二个数。
 但是这两对数据对之间有些关系要讨论。
 比如说序列{1，2，3，4，5}有两对递增的数据对{1，2}和{2，3}，成立；
 序列{5，1，5，5，3，4}首先出现一对递增的数据对{1，5}，和之后出现的{3，4}组成两对数据对，这个为什么成立呢，因为扫描到3时应该更新第一对数据对为{1，3}，于是只要找到比3再大的数就可以了，又降低了条件；
